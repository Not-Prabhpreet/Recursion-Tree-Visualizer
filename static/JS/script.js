...
def merge(left, right):
  ...
  function showError(message) {
    ...
    errorMessage.style.backgroundColor = document.body.classList.contains("dark-mode") ? "#421c1c" : "#ffebee";
    errorMessage.style.border = `1px solid ${document.body.classList.contains("dark-mode") ? "#cf6679" : "#ef5350"}`;
    errorMessage.style.borderRadius = "4px";
    errorMessage.style.color = document.body.classList.contains("dark-mode") ? "#ff8a80" : "#c62828";
  }
  ...
  visualizeBtn.addEventListener("click", function () {
    const functionDefinitionValue = functionDefinition.value.trim();
    const functionCallValue = functionCall.value.trim();

    // Basic validation
    if (!functionDefinitionValue) {
      showError("Please enter a function definition");
      return;
    }
    if (!functionCallValue) {
      showError("Please enter a function call");
      return;
    }
    ...
    .then((data) => {
      if (data.error) {
        ...
        if (svgElement) {
          ...
          showError("Error: Could not create visualization");
        }
      }
    })
    .catch((error) => {
